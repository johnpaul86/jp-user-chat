...
function jp_update_sender_list(){
	if ( !wp_verify_nonce( $_POST['nonce'], "jp_chat_nonce")) {
		echo wp_json_encode("No naughty business please");
		exit;
	}
...
function jp_message_counter(){
	...
	global $wpdb;
	$jp_userId = get_current_user_id();
	$jp_results_count = $wpdb->get_results( $wpdb->prepare( "SELECT id, sender_id, count(id) AS msgs, max(id) as last_id FROM {$wpdb->prefix}jpchat WHERE receiver_id = %d AND recd = '0' GROUP BY sender_id ORDER BY id ASC", $jp_userId ), ARRAY_A);
...
function jp_send_message(){
	...
	if( $jp_send_to > 0 ){
		...
		if(is_int($jp_send_to)){
			if(check_blocked($jp_send_to)){
				...
				if($result_check){
					$jp_results = $wpdb->get_results( $wpdb->prepare( "SELECT sent FROM {$wpdb->prefix}jpchat WHERE id = %d", $wpdb->insert_id ) );
					$jp_msgdAt = '';
	...
	echo wp_json_encode($jp_response);
	wp_die();
}
...
function jp_report_user(){
	...
	if( $block_user_id > 0 ){
		...
		$jp_results = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}jpchat_blockings WHERE block_user_id = %d AND receiver_id = %d AND status = 1", $block_user_id, $jp_send_from ) );
...
function jp_read_messages(){
	...
	if( $jp_send_to > 0 ){
		global $wpdb;
		$jp_send_from = intval( get_current_user_id() );
		$jp_blockResults = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}jpchat_blockings WHERE block_user_id = %d AND receiver_id = %d AND status = 1", $jp_send_to, $jp_send_from ) );
		$jp_blockClass = '';
			...
			<div class="jp-chat-header">
				<div class="jp-chat-header-author-block">
					...
					echo '<span class="name">'.$jp_user_obj->user_nicename.'</span>';
						...
						echo '<span class="name">User Not Exists</span>';
					}
					?>
				</div>
...
function jp_update_msgStatus(){
	...
	global $wpdb;
	$result = $wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->prefix}jpchat SET recd=1 WHERE id <=%d AND sender_id = %d AND receiver_id = %d", $jp_mid, $sender_id, $jp_current_userID ) );
...
function jp_fetchData(){
	...
	if( $jp_send_to > 0 ){
		...
		if( $_POST['triggerOn'] == 'initial' ){
			$jp_results = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}jpchat WHERE ( sender_id = %d AND receiver_id = %d ) OR ( sender_id = %d AND receiver_id = %d ) ORDER BY id ASC", $jp_current_userID, $jp_send_to, $jp_send_to, $jp_current_userID ) );
		}else{
			$jp_results = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}jpchat WHERE sender_id = %d AND receiver_id = %d AND recd='0' ORDER BY id ASC", $jp_send_to, $jp_current_userID ) );
		}
		if( count($jp_results) > 0 ){
			$i = 0;
			foreach($jp_results as $jp_result){

...
function jpchat_settings_init() {
	...
	add_settings_section(
		...
		__( 'These settings are saved in "options" table in database', 'jp-user-chat' ), 'jpchat_settings_page_callback',
		'jpchat'
	);
	...
	add_settings_field(
		'jpchat_restrict_users', // As of WP 4.6 this value is used only internally.
		                        // Use $args' label_for to populate the id inside the callback.
			__( 'Disable User Roles', 'jp-user-chat' ),
...
function jpchat_settings_page_callback( $args ) {
	?>
	<p id="<?php echo esc_attr( $args['id'] ); ?>"><?php esc_html_e( 'Settings Page For JP User Chat plugin', 'jp-user-chat' ); ?></p>
	<?php
}
...
function jpchat_theme_cb( $args ) {
	...
	<select style="padding:10px 30px;" id="<?php echo esc_attr( $args['label_for'] ); ?>" data-custom="<?php echo esc_attr( $args['jpchat_custom_data'] ); ?>" name="jpchat_options[<?php echo esc_attr( $args['label_for'] ); ?>]">
		<option value="dark" <?php echo isset( $options[ $args['label_for'] ] ) ? ( selected( $options[ $args['label_for'] ], 'dark', false ) ) : ( '' ); ?>>
			<?php esc_html_e( 'Dark Theme', 'jp-user-chat' ); ?>
		</option>
 		<option value="light" <?php echo isset( $options[ $args['label_for'] ] ) ? ( selected( $options[ $args['label_for'] ], 'light', false ) ) : ( '' ); ?>>
			<?php esc_html_e( 'Light Theme', 'jp-user-chat' ); ?>
		</option>
	</select>
	<p class="description">
		<?php esc_html_e( 'Dark theme consume less energy than light theme.', 'jp-user-chat' ); ?>
	</p>
	<?php
}
...
function jpchat_restrict_users_cb( $args ) {
	...
	foreach( $roles as $key => $role ){
		...
		<?php esc_html_e( $role['name'], 'jp-user-chat' ); ?>
		</option>
		<?php
...
function jpchat_options_page_html() {
	...
	if ( isset( $_GET['settings-updated'] ) ) {
		...
		add_settings_error( 'jpchat_messages', 'jpchat_message', __( 'Settings Saved', 'jp-user-chat' ), 'updated' );
	}
	...
	<div class="wrap">
		...
		<form action="options.php" method="post">
			...
			submit_button( 'Save Settings', 'jp-user-chat' );
			?>
			
		</form>
		<?php
		global $wpdb;
		if( $wpdb->get_var( "SHOW TABLES LIKE {$wpdb->prefix}jpchat" ) == $jp_tablename ) {
		?>
...
function clear_jp_chat(){
	...
	$result = $wpdb->query( "TRUNCATE TABLE {$wpdb->prefix}jpchat" );
	
	$result2 = $wpdb->query( "TRUNCATE TABLE {$wpdb->prefix}jpchat_blockings" );
...
function drop_jp_tables(){
	...
	global $wpdb;
	$result = $wpdb->query( "DROP TABLE {$wpdb->prefix}jpchat" );
	
	$result2 = $wpdb->query( "DROP TABLE {$wpdb->prefix}jpchat_blockings" );
